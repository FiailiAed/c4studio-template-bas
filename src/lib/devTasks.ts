...
export const TASKS: DevTask[] = [
  ...
  { id: 42, title: 'Commerce feature flags (Funnels, Shops, Booking Links)', status: 'planned', priority: 'P1', version: 'v0.0.5', category: 'feature-flags', notes: 'No funnelsEnabled/shopsEnabled/bookingLinksEnabled flags exist anywhere. Must mirror blogEnabled pattern: schema field in appSettings, settings.ts upsert arg, settings UI toggle, page redirect logic, Header/Footer nav filter.' },
  ...
  { id: 57, title: 'Email unsubscribe / opt-out mechanism',               status: 'overlooked', priority: 'P0', version: 'v1.0', category: 'compliance', notes: 'CAN-SPAM and GDPR require an unsubscribe mechanism in any marketing or promotional email. All three agent sequences (3, 4, 5) send email with no unsubscribe link. This is a legal requirement for any production deployment.' },
  ...
  { id: 63, title: '500 / server error page',                             status: 'overlooked', priority: 'P2', version: 'v1.0', category: 'public-ui',  notes: '/404.astro exists but there is no error boundary or 500 fallback for Astro SSR failures or unhandled Convex errors. Vercel shows a generic error page in these cases.' },
  ...
  { id: 69, title: 'Security headers in vercel.json',                     status: 'overlooked', priority: 'P3', version: 'v1.0', category: 'security',   notes: 'No vercel.json with response headers. X-Frame-Options (clickjacking), X-Content-Type-Options (MIME sniffing), Referrer-Policy, and a basic Content-Security-Policy are expected for any production deployment.' },
  { id: 70, title: 'appSettings schema fields for commerce flags',        status: 'overlooked', priority: 'P1', version: 'v0.0.5', category: 'convex',   notes: 'funnelsEnabled, shopsEnabled, bookingLinksEnabled do not exist in convex/schema.ts or settings.ts. These must be added before task #42 (commerce feature flags) can be implemented.' },
