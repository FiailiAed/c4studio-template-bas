...
export const TASKS: DevTask[] = [
  ...
  // ─── PLANNED — v0.0.5 ────────────────────────────────────────────────────────
  { id: 41, title: 'Design system color picker fix',                      status: 'complete', priority: 'none', version: 'v0.0.5', category: 'admin-ui',      notes: 'Fixed bidirectional sync between type=color, hex text input, hidden POST field, and live swatch preview using data-* attributes and a single applyColor() helper. Saved values now round-trip correctly on reload.' },
  ...
  { id: 81, title: 'Email: Add padding to top and bottom of email',         status: 'complete', priority: 'none', category: 'email',       notes: 'Added padding:32px 24px to body of all outbound HTML emails. Fixed in emailHtml() helper in nurturingActions.ts, reactivationActions.ts, reviewActions.ts. Also upgraded contact/booking confirmation emails in email.ts to full document structure with header, card wrapper, and inline styles.' },
  { id: 83, title: 'Products: Stripe Product Management',                    status: 'complete', priority: 'none', category: 'payments',    notes: 'Added convex/stripeProducts.ts ("use node" actions) with listProducts, createProduct, updateProduct, archiveProduct and public admin wrappers. Added Products tab to /admin/billing: live product table, create form (name, description, price in dollars), inline edit per row, archive with confirm dialog. Stripe API calls use expand default_price.' },
  { id: 85, title: 'Expand /admin/contacts to directory with per-contact detail view', status: 'complete', priority: 'none', version: 'v0.0.5', category: 'admin-ui', notes: 'Converted flat contacts.astro to contacts/ directory. contacts/index.astro keeps list view with link to detail. contacts/[id].astro shows full message, contact info card, read/unread toggle, delete, mailto reply, and Agent 5 nurturing log for that contact. Added contacts.getById query and markUnread mutation to convex/contacts.ts. Added getLogsForContact query to convex/nurturing.ts.' },

  { id: 84, title: 'Native Raffle System — entries, tracking, and winner selection', status: 'complete', priority: 'none', version: 'v1.0', category: 'agents', notes: 'Added raffles + raffleEntries Convex tables. convex/raffles.ts with list, getById, getEntries, create, update, remove, addEntry, drawWinner, closeRaffle, enterRaffle. Tokenized GET /api/raffle/enter?raffleId&contactId&token (HMAC: raffle:id:contactId). /raffle/entered confirmation page. /admin/raffles index + [id] detail with edit/close/draw-winner/delete and entrant list with winner highlight. Raffles card added to admin hub. devnotes Step 31 added.' },

  // ─── PLANNED — v0.0.9 ────────────────────────────────────────────────────────
  { id: 86, title: 'Hamburger menu only works once on mobile',              status: 'planned', priority: 'P1', version: 'v0.0.9', category: 'public-ui', notes: 'Root cause: the <script> in Header.astro runs as an ES module. Browsers cache and execute modules only once per session, so after the first page navigation the addEventListener never re-attaches to the new DOM. Fix: add is:inline directive to the Header script block so it executes as a regular inline script on every page load. One-line change in src/components/marketing/Header.astro.' },
  { id: 87, title: 'Admin UI — mobile and small screen responsive design',  status: 'planned', priority: 'P1', version: 'v0.0.9', category: 'admin-ui',  notes: 'Admin portal is not usable on phones or small screens. Issues: fixed-width data tables overflow viewport, 4-col grids do not stack, admin header nav wraps badly. Fix across all admin pages: wrap all tables in overflow-x-auto, change grid-cols-4 to grid-cols-1 sm:grid-cols-2 lg:grid-cols-4, ensure action buttons stack or shrink on mobile, make admin header readable at 375px. Consider a collapsible sidebar nav for the admin section. This touches every admin page — do in a single sweep.' },
  { id: 88, title: 'Raffle: public entry form page and copy link in admin',  status: 'planned', priority: 'P1', version: 'v0.0.9', category: 'agents',    notes: 'Current /api/raffle/enter is HMAC-tokenized — only works from agent emails. Need a public non-tokenized entry form at /raffle/[id] (SSR page, no auth). Form: first name, last name, email. On POST: upsert contact record (find by email or create with entryMethod "public_form"), call raffles.addEntry. Redirect to /raffle/entered. On /admin/raffles/[id], add a "Public Entry Link" card with the full URL and a copy-to-clipboard button. This URL is what admins paste into appSettings.raffleLink. Add entryMethod: "public_form" to raffleEntries schema union.' },
  { id: 89, title: 'Feedback form — public form, admin CRUD, response view', status: 'planned', priority: 'P1', version: 'v0.0.9', category: 'admin-ui',  notes: 'New Convex tables: feedbackForms (title, slug, description, published: bool) and feedbackResponses (formId, contactName, contactEmail, rating: 1-5 number, message, submittedAt). New module convex/feedback.ts: list, getBySlug, create, update, remove, submitResponse, listResponses. Admin pages: /admin/feedback/index.astro (list + create with slug auto-gen), /admin/feedback/[id].astro (edit form, view all responses in a table, copy public URL button). Public page: /feedback/[slug].astro (renders name, email, rating 1-5 stars, message fields). On submit: POST to same page → calls feedback.submitResponse → redirect to /feedback/[slug]?submitted=1 (shows thank-you inline, no separate page). Add feedbackForms to api.d.ts and schema.ts. Update appSettings.feedbackFormLink to be populated from this system.' },
  { id: 90, title: 'Referral system — per-contact codes, booking attribution', status: 'planned', priority: 'P1', version: 'v0.0.9', category: 'commerce',  notes: 'Design decision: extend booking links, do not build a new link system. Schema changes: add referralCode: v.optional(v.string()) to contacts table (short unique 8-char code, generated on demand via contacts.generateReferralCode mutation). Add referredBy: v.optional(v.string()) to bookings table. Logic: when /api/bookings POST receives query param ref=CODE, look up the contact with that referralCode and store it on the booking record. Admin UX: /admin/contacts/[id] gets a "Referral Link" section — shows a "Generate Link" button (calls generateReferralCode if not set), then displays /booking-links/[defaultSlug]?ref=[code] with copy button. /admin/booking-links/[id] shows a "Referrals" count badge on bookings that have referredBy set. The generated share link goes into appSettings.referralShareLink. No new tables needed. No new public pages. bookingLinks query updated to accept ?ref= param passthrough at the BookingWidget level.' },
  { id: 91, title: 'Shops: select products from Stripe account in shop editor', status: 'planned', priority: 'P1', version: 'v0.0.9', category: 'commerce',  notes: 'In /admin/shops/[id] the Add Product form requires the admin to type a raw Stripe Price ID. Replace with: fetch all Stripe products on page load via client.action(api.stripeProducts.adminListProducts, {}), render a <select> dropdown of products (showing name + price). When admin selects a product, JS auto-fills the name, description, price (cents→dollars), and stripePriceId hidden input. Admin can still manually override any field. The stripePriceId field becomes a hidden input populated from the dropdown selection rather than a free-text field. Existing shopItems schema and save mutation are unchanged — only the form UX changes in src/pages/admin/shops/[id].astro.' },
  { id: 92, title: 'Agent Variables: smart URL picker for system URLs',       status: 'planned', priority: 'P2', version: 'v1.0',   category: 'admin-ui',  notes: 'In /admin/settings → Agent Variables section, the four URL-type fields (defaultBookingLink, raffleLink, feedbackFormLink, referralShareLink) currently have plain text inputs. Add a <select> dropdown above each URL input that lets admin pick from known system URLs: booking link slugs from api.bookingLinks.list, raffle entry URLs from api.raffles.list (as /raffle/[id]), feedback form URLs from api.feedbackForms.list (as /feedback/[slug]), and a "Custom URL" option. On select, populate the text input with the chosen URL. If admin types directly, the dropdown resets to "Custom URL". The text input remains the source of truth that POSTs to the server. Requires feedbackForms (task 89) to be complete for the feedback picker to work.' },
